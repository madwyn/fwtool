...
int main(int argc, char **argv) {
    ...
    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_MODE_ADV_HELP_F, MSG_MODE_ADV_HELP, "b", &adv_help);
    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_MODE_EXTRACT_F , MSG_MODE_EXTRACT , "b", &extract);
    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_MODE_CREATE_F  , MSG_MODE_CREATE  , "b", &create);
    ...
    // input parameters
    ret |= cargo_add_group(cargo, (cargo_group_flags_t)0, OPT_GRP_IO, "Input and output parameters", "They have different meanings depending on working mode.");

    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_IN_F , MSG_IO_IN , "s", &input);
    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_OUT_F, MSG_IO_OUT, "s", &output);
    ...
    ret |= cargo_add_option(cargo, (cargo_option_flags_t)0, OPT_VERBOSE_F, MSG_OPT_VERBOSE, "b", &verbose);
    ...
    if (!cargo_parse(cargo, (cargo_flags_t)0, 1, argc, argv)) {
        if (adv_help) {
            ...
            cargo_group_set_flags(cargo, OPT_GRP_OPTION, (cargo_group_flags_t)0);
